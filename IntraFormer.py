...
def evaluate_ensemble_close_prediction(ensemble_members, full_scaled, seq_len, batch_size, device, scaler_close, test_data, symbol):
...
if __name__ == "__main__":
    ...
    if all_predictions_dfs:
        pd.concat(all_predictions_dfs, ignore_index=True).to_csv(
            os.path.join(output_dir, "IntraFormer_all_equity_true_vs_pred2.csv"), index=False
        )
        print(f"\nSaved predictions: {output_dir}/IntraFormer_all_equity_true_vs_pred.csv")

    if all_metrics_list:
        pd.DataFrame(all_metrics_list).to_csv(
            os.path.join(output_dir, "IntraFormer_all_equity_metrics2.csv"), index=False
        )
        print(f"Saved metrics: {output_dir}/IntraFormer_all_equity_metrics.csv")
